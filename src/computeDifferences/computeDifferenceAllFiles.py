...
"""
This script processes event data from multiple CSV files, computes statistical summaries, and generates visualizations.
Modules:
...
Variables:
    all_events_data (DataFrame): DataFrame to store concatenated event data from all CSV files.
    excluded_events_boxplot (list): List of event types to exclude from the boxplot.
    df_filtered_boxplot (DataFrame): DataFrame containing filtered event data for the boxplot.
    boxplot_data (list): List of series containing 'difference' values for each event type.
    boxplot_filename (str): Path to save the boxplot image file.
    summary_stats_all_games (DataFrame): DataFrame containing mean and standard deviation of 'difference' for each event type.
    overall_mean (float): Overall mean of 'difference' across all events.
    overall_std (float): Overall standard deviation of 'difference' across all events.
    overall_stats (DataFrame): DataFrame containing overall mean and standard deviation.
    final_summary_stats_all_games (DataFrame): DataFrame containing combined summary statistics for each event type and overall.
...
Execution:
    ...
    4. Computes and saves summary statistics (mean and standard deviation) of 'difference' for each event type and overall.
"""
...
df_filtered_boxplot = all_events_data[~all_events_data['event_type'].isin(excluded_events_boxplot)]
...
boxplot_filename = os.path.join(output_dir_box, "boxplot_differences_all_games.png")
...
summary_stats_all_games = all_events_data.groupby('event_type')['difference'].agg(['mean', 'std']).reset_index()
...
final_summary_stats_all_games = pd.concat([summary_stats_all_games, overall_stats], ignore_index=True)

summary_filename = os.path.join(output_dir_sum, "summary_statistics_all_games.csv")
