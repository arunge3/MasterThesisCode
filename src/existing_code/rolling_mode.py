...
def _mode(signal: np.array) -> np.array:
    """
...
def rolling_mode(signal: np.array, window_size: int) -> np.array:
    """
    ...
    modes = _mode(windows)
    rolled_signal[int(window_size / 2): -
                  int(window_size / 2)] = modes.squeeze()
